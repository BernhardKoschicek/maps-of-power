...
- **Function Parameter Alignment**: If a function call wraps onto multiple lines, the parameters must use a hanging indent (double indentation from the `def` keyword, usually 8 spaces), and the closing parenthesis should remain on the same line as the last parameter.
